...
// Function to check individual characters
const checkEachCharacter = (inputedWord) => {
  
  for (let idx in inputedWord) {
    let char = inputedWord[idx];
    gridItems[index++].textContent = char;
    if (correctWord.includes(char)) {
      if (inputedWord[idx] === correctWord[idx]) {
        gridItems[index - 1].style.cssText = "background-color: green; color: white;"
      } else {
        gridItems[index - 1].style.cssText = "background-color: yellow; color: black;"
      }
    } else {
      gridItems[index - 1].style.cssText = "background-color: gray; color: white;"
    }
  }
}

// Function to check if the inputed word is correct or not
const checkInputedWord = () => {

  const inputedWord = inputField.value;

  if (correctWord === inputedWord) {
    message.textContent = `Congratulations! You guessed the word!`
    for(char of inputedWord) {
      gridItems[index++].textContent = char;
      gridItems[index - 1].style.cssText = "background-color: green; color: white;"
    }
    inputField.disabled = true;
  } else {
    checkEachCharacter(inputedWord);
  }
}
